...
// ====== helpers & constants (copied from your server) ======
...
const SYNONYMS = new Map([
  ['main code','Item-Code'],['pos description','Item-POS description'],
  ['totalizer-number','Sub-department-Number'],['totalizer-description','Sub-department-Description'],
...
function normalizeHeader(h){ if(!h&&h!==0)return''; let s=String(h).trim();
  if((s.startsWith('"')&&s.endsWith('"'))||(s.startsWith("'")&&s.endsWith("'"))) s=s.slice(1,-1).trim();
  s=s.replace(/^"+|"+$/g,'').trim().replace(/\u00A0/gu,' ')
    .replace(/[\u2010\u2011\u2012\u2013\u2014\u2212]/gu,'-')
...
const CANONICAL_FROM_NORM = (()=>{ const map=new Map();
...
function pad13(s){ const d=String(s??'').replace(/\D+/g,''); return d.padStart(13,'0'); }
function numberOrZero(v){ if(v==null||v==='')return 0;
  const s=String(v).trim().replace(/\s+/g,'').replace(/,/g,'').replace(/%$/,'').replace(/^\((.*)\)$/,'-$1');
  const n=Number(s); return Number.isFinite(n)?n:0;
}
function coerceCentury(d){ const y=d.getFullYear(); if(y>=100)return d; const pivot=(y>=70)?1900:2000; const adj=new Date(d); adj.setFullYear(pivot+y); return adj; }
function parseDateToISO(v){
  if(!v) return null; const s=String(v).trim();
  const fmts=['yyyy-MM-dd','MM/dd/yyyy','M/d/yyyy','M/d/yy','dd/MM/yyyy','d/M/yyyy','d/M/yy','yyyy/M/d','dd-MMM-yy','dd-MMM-yyyy'];
  for(const fmt of fmts){ const d=parseDateFns(s,fmt,new Date()); if(isValid(d)) return formatDate(coerceCentury(d),'yyyy-MM-dd'); }
...
function validateHeadersFromRaw(rawObj){
  const present=new Set(); for(const k of Object.keys(rawObj)){ const canon=CANONICAL_FROM_NORM.get(normalizeHeader(k)); if(canon) present.add(canon); }
  const missingAll=REQUIRED_HEADERS.filter(h=>!present.has(h));
  const missingMin=MIN_HEADERS.filter(h=>!present.has(h));
  return { ok: missingMin.length===0, missing: missingAll };
}
async function parseCsv(filePath){
  const raw=[]; await new Promise((res,rej)=>{ fs.createReadStream(filePath)
    .pipe((csvParse)({ columns:true, skip_empty_lines:true, bom:true, trim:true }))
    .on('data',(rec)=>raw.push(rec)).on('end',res).on('error',rej);});
  if(raw.length===0) return { rows:[], missing:REQUIRED_HEADERS };
  const {ok,missing}=validateHeadersFromRaw(raw[0]); if(!ok) return { rows:[], missing };
  const rows=raw.map(remapRowToCanonical).map(fillDefaultsToRequired); return { rows, missing:[] };
}
async function parseXlsb(filePath){
  const wb=XLSX.readFile(filePath,{ cellDates:false }); const sheet=wb.SheetNames[0]; const ws=wb.Sheets[sheet];
  const raw=XLSX.utils.sheet_to_json(ws,{ raw:false, defval:'', header:0 });
  if(raw.length===0) return { rows:[], missing:REQUIRED_HEADERS };
  const {ok,missing}=validateHeadersFromRaw(raw[0]); if(!ok) return { rows:[], missing };
  const rows=raw.map(remapRowToCanonical).map(fillDefaultsToRequired); return { rows, missing:[] };
}
...
async function processUploadJob(filePath, originalName){
  ...
  if(parsed.missing?.length){ const err=new Error('Header validation failed: '+parsed.missing.join(', ')); err.code='HEADERS'; throw err; }
  ...
  for(const r of parsed.rows){
    ...
    if(batch.length>= (Number(process.env.BATCH_SIZE||1000))) { insertManyTxns(batch); batch.length=0; }
  }
  if(batch.length) { insertManyTxns(batch); batch.length=0; }
  if(subPairs.size){ const pairs=Array.from(subPairs).map(s=>{ const [no,desc]=s.split('::'); return [Number(no), desc]; }); upsertSubdepartments(pairs); }
  ...
  return { fileName: originalName, rowsParsed: parsed.rows.length, inserted: insertedTotal, ignored, sampleDates: Array.from(sampleDates).sort(), elapsedMs: Date.now()-started };
}
// ====== run job ======
(async () => {
